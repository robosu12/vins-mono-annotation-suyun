...
void MarginalizationInfo::addResidualBlockInfo(ResidualBlockInfo *residual_block_info)
{
    ...
    std::vector<double *> &parameter_blocks = residual_block_info->parameter_blocks; //优化变量数据
    // 其中parameter_block_sizes每个优化变量块的变量大小，以IMU残差为例，为[7,9,7,9]
    std::vector<int> parameter_block_sizes = residual_block_info->cost_function->parameter_block_sizes();

    // static_cast 可以被用于强制隐形转换,它最接近于C-style的转换。

    // reinterpret_cast主要是将数据从一种类型的转换为另一种类型,将数据以二进制存在形式的重新解释,转换前后没有数位损失;
    // 比如：
    // int i;
    // char *p = "This is an example.";
    // i = reinterpret_cast<int>(p);
    // 此时结果，i与p的值是完全相同的。reinterpret_cast的作用是说将指针p的值以二进制（位模式）的方式被解释为整型，并赋给i，
    // i 也是指针，整型指针；一个明显的现象是在转换前后没有数位损失。

    for (int i = 0; i < static_cast<int>(residual_block_info->parameter_blocks.size()); i++)
    {
        double *addr = parameter_blocks[i];
        int size = parameter_block_sizes[i];
        // map<优化变量内存地址，localSize>
    ...
    for (int i = 0; i < static_cast<int>(residual_block_info->drop_set.size()); i++)
    {
        ...
        // map<待marg的优化变量内存地址，在parameter_block_size中的id,以double为单位>
