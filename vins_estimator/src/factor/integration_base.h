...
class IntegrationBase
{
  public:
    ...
    //***增加理解：预积分的结果实质是对imu测量量的积分:
    //1.即对加速度积分，得到两帧间由于加速度造成的位移增量以及速度增量，而不是两帧间的位移增量,因为两帧间的位移增量还跟当前系统的速度有关
    //即积分得到的位移是由加速度导致的位移，加上当前系统的速度造成的位移，才得到两帧间的位移增量；
    //速度增量即为加速度导致的速度增量；
    //2.即对角速度进行积分，得到两帧间的角度增量；
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        // 根据上一帧和当前帧加速度与角速度数据进行预积分，计算两帧图像间的 P,V,Q 增量
        ...
        delta_q.normalize();
        sum_dt += dt;
        acc_0 = acc_1; //保存为下一帧加速度
        gyr_0 = gyr_1; //保存为下一帧角速度
     
    }

    // evaluate 计算IMU测量模型的残差;
    //从大jacobian中分别取出p,v,q,ba,bg相对于ba,bg的jacobian，然后通过一阶近似对预积分值（测量值）进行估计，也即校正；
