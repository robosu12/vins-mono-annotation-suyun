...
ProjectionFactor::ProjectionFactor(const Eigen::Vector3d &_pts_i, const Eigen::Vector3d &_pts_j) : pts_i(_pts_i), pts_j(_pts_j)
{
    ...
    b2 = a.cross(b1); //交叉相乘，叉乘
    //因为视觉残差的自由度为 2，因此将视觉残差投影到正切平面上，b1,b2 为正切平面上的任意两个正交基
...
bool ProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    //视觉残差是重投影误差：对于第 l个路标点 P，将 P 从第一次观看到它的第 i个相机坐标系，转换到当前的第 j 个相机坐标系下，
    //与在第 j 个相机归一化坐标系下观测到的位置做差，
    Eigen::Vector3d pts_camera_i = pts_i / inv_dep_i;       // pt in ith camera frame, 归一化平面（相机坐标系）
    Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic;   // pt in ith body frame(imu坐标系)
    Eigen::Vector3d pts_w = Qi * pts_imu_i + Pi;            // pt in ith world frame（i帧的世界坐标系）
    Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj);// pt in jth body frame(imu坐标系)
    Eigen::Vector3d pts_camera_j = qic.inverse() * (pts_imu_j - tic); // pt in jth camera frame, 归一化平面（相机坐标系）
    Eigen::Map<Eigen::Vector2d> residual(residuals);

#ifdef UNIT_SPHERE_ERROR 
    // 把归一化平面上的重投影误差投影到Unit sphere上的好处就是可以支持所有类型的相机
    // 单位椭圆和正切空间，与重投影误差有什么关系，是否因为球形相机模型？（suyun?）
    residual =  tangent_base * (pts_camera_j.normalized() - pts_j.normalized()); // 求取切平面上的误差
#else
    double dep_j = pts_camera_j.z();
    residual = (pts_camera_j / dep_j).head<2>() - pts_j.head<2>();  // 求取归一化平面上的误差
#endif
    //固定的协方差矩阵，归一化平面的标准差为 1.5/f，即像素标准差为1.5；
    residual = sqrt_info * residual;    // 转成与量纲无关的马氏距离

    //根据视觉残差公式，我们可以得到相对于各优化变量的雅克比矩阵；
    if (jacobians)
    {
