...
{
    cv::Point2f best_pt;
    int bestDist = 128;  // 非匹配点的汉明距离呈现均值为128比特征的高斯分布（brief描述子设置为256维）
    int bestIndex = -1;
    for(int i = 0; i < (int)descriptors_old.size(); i++)
    {
        if(!inAera(keypoints_old[i].pt, center_cur, area_size))
            continue;   //若候选帧特征点位置超过此范围，则直接跳过，因为相似度高的帧，同一特征点的位置不会距离太远；

        int dis = HammingDis(window_descriptor, descriptors_old[i]); //两个二进制描述子相与，计算描述子汉明距离
        if(dis < bestDist)
        {
            bestDist = dis;
            bestIndex = i;
                                      ...
                                      const camodocal::CameraPtr &m_camera)
{
    if (measurements_old.size() >= 8)
    {
        ...
        for (int i = 0; i < (int)measurements_matched.size(); i++) //根据相机模型去除畸变
        {
        ...
        reduceVector(measurements_old_norm, status);  //根据匹配的状态，剔除外点
                           ...
                           const camodocal::CameraPtr &m_camera)
{
    ...
    //遍历当前关键帧的每一个特征点，与一定范围内的候选帧的特征点进行匹配，因为相似度高的帧，同一特征点的位置不会距离太远；
    for(int i = 0; i < (int)window_descriptors.size(); i++)
    {
        cv::Point2f pt(0.f, 0.f);
        //以当前特征点位置为中心，200为半径，与在此范围内的候选帧特征点进行匹配计算，pt为匹配到的候选帧的点
        if (searchInAera(measurements[i], 200, window_descriptors[i], descriptors_old, keypoints_old, pt))
          status.push_back(1);
        else
          status.push_back(0);          //根据匹配的状态，剔除无匹配的点
        measurements_old.push_back(pt); //measurements_old 为匹配到的候选帧中的点
    }
    measurements_matched = measurements;
    features_id_matched = features_id;
    point_clouds_matched = point_clouds;
    reduceVector(measurements_old, status);     //measurements_old 为匹配到的候选帧中的点
    reduceVector(measurements_matched, status); //measurements_matched 为匹配到的当前帧中的点，和候选帧中点一一对应
    reduceVector(features_id_matched, status);  //features_id_matched 为匹配到的当前帧的点的id
    reduceVector(point_clouds_matched, status); //point_clouds_matched 为匹配到的当前帧的点的3D坐标
}
...
{
    BRIEF::bitset xor_of_bitset = a ^ b;  //两个二进制描述子相与，计算描述子汉明距离
